...
public class ThreadedSolver {
    public static void main(String[] args) {
        ...
        int     numThreads = 1;        // Use this many threads to search
        ...
        if(args.length == 0) {
            ...
            System.err.println("  threads   - use this many threads to solve each polynomial  (default " + numThreads + ")");
            ...
            System.err.println("  file.poly - one+ polynomial files (#terms then coefficient and exponent of each on separate lines)");
            System.exit(0);
        }
        ...
        while(++index < args.length) {
            ...
            else if(args[index].equals("--threads")) numThreads = Integer.parseInt  (args[++index]);
        ...
        System.out.println("\nrange " + min + " to " + max + ", " + slices + " slices, precision = " + precision + "\n");
        Thread[] threads = new Thread[polynomials.size()];
        
        final double fmin = min;
        final double fmax = max;
        final double fslices = slices;
        final int    fnumThreads = numThreads;
        final double fprecision = precision;

        int threadID = 0;
        for(var file : polynomials.keySet()) {
            final Polynomial p = polynomials.get(file);
            Thread t = new Thread(() -> {
                p.solve(fmin, fmax, fnumThreads, fslices, fprecision);
        ...
        for(threadID=0; threadID<threads.length; ++threadID) {
            try {
